...
def initial_date_choice_handler(call):
    ...
    if data.startswith('DEFAULT_DATE_'):
        ...
        bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=f"📅 Attendance Date: {selected_date_str}")
...
def show_date_selector(chat_id, year=None, month=None, message_id=None):
    ...
    nav_row = [
        types.InlineKeyboardButton("⬅️", callback_data=f"CALENDAR_NAV_{prev_month_date.year}-{prev_month_date.month}"),
        types.InlineKeyboardButton("Today", callback_data=f"CALENDAR_TODAY_{now.year}-{now.month}-{now.day}"),
        types.InlineKeyboardButton("➡️", callback_data=f"CALENDAR_NAV_{next_month_year}-{next_month_month}")
    ]
    markup.add(*nav_row)
    ...
    if message_id:
        ...
        bot.send_message(chat_id, "📅 Select an Attendance Date:", reply_markup=markup)
...
def calendar_callback_handler(call):
    ...
    if action == "NAV":
        ...
        bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=f"📅 Attendance Date: {today_date_str}")
...
#    markup.add(*SESSIONS)
#
#    msg = bot.send_message(chat_id, "🕒 Select Session:", reply_markup=markup)
...
def _display_session_selection_for_update(chat_id):
    ...
    markup.add(*SESSIONS)
    markup.add("Back", "Cancel")
    msg = bot.send_message(chat_id, "🕒 Select session to update:", reply_markup=markup)
...
def _display_subject_selection(chat_id):
    ...
    markup.add(*SUBJECTS)
    markup.add("Back", "Cancel")
    msg = bot.send_message(chat_id, "📚 Select Subject:", reply_markup=markup)
...
def process_session(message):
    ...
    if user_data.get(chat_id, {}).get('action_type') == 'record_new':
        ...
        if session_already_exists:
            ...
            bot.send_message(
                chat_id,
                f"⚠️ Attendance for Session {selected_session} on {current_date} already exists. "
...
def process_subject(message):
    ...
    msg = bot.send_message(
        chat_id,
        "✍️ Enter ABSENTEES (comma separated)\nExample:\nAfreen, Amina",
        reply_markup=markup
    )
...
def process_absentees(message):
    ...
    if found_row_index != -1:
        ...
        bot.send_message(chat_id, "✅ Attendance updated successfully! (Note: Subject and absentees for this session on this date have been updated.)")
    else:
        # Append new row
        sheet.append_row(new_row_data)
        bot.send_message(chat_id, "✅ Attendance saved successfully!")
...
def handle_continue_attendance_choice(message):
    ...
    if choice == "record new session":
        ...
        bot.send_message(
            chat_id,
            "🎉 All attendance recorded for today. You can type /start or /attendance to begin a new entry.",
...
def handle_cancel_flow(chat_id):
    ...
    bot.send_message(chat_id, "❌ Attendance process cancelled.", reply_markup=types.ReplyKeyboardRemove())
...
def check_for_commands(message, current_handler_name):
    ...
    if text == 'Cancel':
        ...
        bot.send_message(chat_id, "↩️ Going back...", reply_markup=types.ReplyKeyboardRemove())
